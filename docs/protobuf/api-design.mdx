...
- `PayLightning`: pay the swap invoice and persist the preimage to the swap record.
- `ClaimAsset`: claim the Liquid HTLC (asset acquisition) using the stored preimage and buyer signature.
...
- `PayLightning` and `ClaimAsset` are idempotent per `swap_id` (returns persisted results when already done).

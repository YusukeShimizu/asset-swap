...
- **Swap server**
  ...
  - Holds seller inventory and buyer wallets in this minimal regtest-oriented setup.
- **CLI user**
...
The client sends a `PayLightningRequest`:

- `swap_id`: UUID string.
- `payment_timeout_secs`: how long to wait for the preimage (0 uses a server default).

The server returns a `PayLightningResponse` containing:
...
The client sends a `ClaimAssetRequest`:

- `swap_id`: UUID string.
- `claim_fee_sats`: claim tx fee in sats (0 uses a server default).

The server returns a `ClaimAssetResponse` containing:
...
Note: The server updates swap status to `PAID` / `CLAIMED` only when `PayLightning` / `ClaimAsset`
...
grpcurl \
  ...
  127.0.0.1:50051 \
  ln_liquid_swap.v1.SwapService/PayLightning
...
grpcurl \
  ...
  127.0.0.1:50051 \
  ln_liquid_swap.v1.SwapService/ClaimAsset
```
